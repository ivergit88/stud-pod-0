...
```json
{
  ...
  "additionalData": {
    ...
    "skills": ["Программирование", "Веб-разработка"]
  },
  "password": "secret"
}
```
...
`description` — направление подготовки (поле «Направление подготовки» на сайте, необязательное
для API). `409` — email уже занят.

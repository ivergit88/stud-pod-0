...
- `/эксперт` — локальная демонстрация без регистрации. Эксперт переключается между учреждением и студентом, а созданные задачи и действия остаются только в `localStorage` текущего браузера.

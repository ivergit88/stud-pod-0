...
- `npm run smoke:admin` — изолированная проверка создания администратора и блокировки пользователя
- `npm run db:import-evidence -- --database ./database.sqlite` — проверить состав импорта доказательной базы без изменений
...
- `ADMIN_EMAIL` — email владельца платформы (по умолчанию `ershovivan2802@yandex.ru`)
- `ADMIN_SETUP_TOKEN` — отдельный секрет минимум из 16 символов для первичной настройки администратора
...
Команда `npm run local` сама печатает кликабельные ссылки на главную, экспертный режим, вход и настройку администратора.
...
## Экспертный и административный режимы

- `/эксперт` — локальная демонстрация без регистрации. При первом входе автоматически разворачивается готовая демо-сцена: 5 задач на разных стадиях (выполненные соло и командой, задача на приёмке, открытая для пробного отклика), команда из 3 человек, баллы, мероприятие и магазин с обменом баллов на мерч. Эксперт переключается между учреждением и студентом, а созданные задачи и действия остаются только в `localStorage` текущего браузера; кнопка «Начать демонстрацию заново» восстанавливает исходные примеры.
- `/администратор/настройка` — защищённое первичное создание или восстановление администратора. Нужен `ADMIN_SETUP_TOKEN` из серверного `.env`.
- `/администратор` — пользователи, блокировки, все публикации, редактирование открытых задач, журнал действий и последние ошибки API.

После настройки администратор входит на обычной странице `/вход` по email `ershovivan2802@yandex.ru` и выбранному паролю. Обычная регистрация с этим email запрещена.

Пошаговый вход администратора:

1. Запустите `npm run local`.
2. Откройте напечатанную ссылку `/администратор/настройка`.
3. Возьмите значение `ADMIN_SETUP_TOKEN` из локального `.env`, задайте пароль администратора.
4. В дальнейшем открывайте `/вход` и используйте `ershovivan2802@yandex.ru` и заданный пароль.

## Восстановление доказательной базы

Импорт студентов, учреждений, задач, мероприятий и форм выполняется идемпотентно. Без `--apply` команда только показывает объём:

```bash
npm run db:import-evidence -- --database ./database.sqlite
```

Применить импорт и удалить только пользователей с системными ID `demo-student-*`:

```bash
npm run db:import-evidence -- --database ./database.sqlite --clear-demo --apply
```

Импортированные участники получают технические адреса вида `sp-0001@participants.stud-pod.local` и случайные пароли. Единственный файл с исходными паролями создаётся локально как `imported-user-credentials.csv`, имеет права `600` и исключён из Git и деплой-архива. Пароли в базе хранятся только в виде bcrypt-хешей и не отображаются в админ-панели.

Импорт также фиксирует итог пилота: комплект мерча Университета Неймарк, его обмен Александром Беловым за 1200 баллов и статус «выдано». Три проектные сессии импортируются с датами 03.05.2026, 14.07.2026 и 04.08.2026; прошедшие события отображаются серыми завершёнными карточками.

Для инструментов импорта один раз выполните:

```bash
npm run setup:import-tools
```
